...
import { asideLeftContentMock } from './mock/aside-left.mock';
  ...
  return (
    <aside className="hidden lg:block flex-1 sticky top-20 overflow-y-auto" style={{ height: 'calc(100vh - 64px)' }}>
      <section className="grid grid-cols-2 gap-2">
        <div className="rounded-xl flex transition duration-200 bg-primary-6 text-white hover:brightness-105 flex-col">
          <div className="">
        ...
        <ul className="text-sm font-medium space-y-2">
          {asideLeftContentMock?.asideLeftMenu.map((item, index) => {
            ...
            return (
              <li key={id}>
                <Button
                  className="rounded-md  flex justify-start items-center gap-2  h-10 text-sm px-4 text-primary-6 dark:text-primary-3 hover:bg-primary-6/20 dark:hover:bg-primary-6/20 break-words transition duration-200 py-2"
                  type="link"
                  link={link}
                  title={name}
                >
